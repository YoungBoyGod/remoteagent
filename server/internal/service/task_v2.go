...
func (s *Service) CreateTask(req api.TaskCreateRequest) (*api.TaskCreateResponse, error) {
	...
	// 校验 TargetAgentID 对应的 agent 是否存在
	if req.Schedule != nil && req.Schedule.TargetAgentID != "" {
		s.mu.Lock()
		_, ok := s.agents[req.Schedule.TargetAgentID]
		s.mu.Unlock()
		if !ok {
			return nil, fmt.Errorf("agent not found: %s", req.Schedule.TargetAgentID)
		}
	}
...
func taskRowToDetail(row store.TaskRow) api.TaskDetail {
	...
	if len(row.Payload) > 0 {
		...
		if detail.Payload.Env != nil {
			if v := strings.TrimSpace(detail.Payload.Env["RA_SUBMITTER"]); v != "" {
				detail.Submitter = v
			} else if v := strings.TrimSpace(detail.Payload.Env["SUBMITTER"]); v != "" {
				detail.Submitter = v
			} else if v := strings.TrimSpace(detail.Payload.Env["CREATED_BY"]); v != "" {
				detail.Submitter = v
			}
		}
	}
