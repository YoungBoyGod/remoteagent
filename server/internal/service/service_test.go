...
// TestProcessTaskReport_StderrForcesFailed 测试严格规则：
// 即使 agent 上报 status=success，只要 stderr 非空，服务端也必须强制按 failed 处理。
func TestProcessTaskReport_StderrForcesFailed(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatalf("new sqlmock: %v", err)
	}
	...
	svc := service.New(db)
	svc.SetAgent("agent-1", &model.AgentRecord{
		AgentID: "agent-1", RunningTasks: map[string]struct{}{"task-stderr": {}},
	})
	...
	mock.ExpectBegin()
	mock.ExpectExec(regexp.QuoteMeta("insert into tasks(")).
		WithArgs("task-stderr", "agent-1", "failed", sqlmock.AnyArg(), sqlmock.AnyArg()).
		WillReturnResult(sqlmock.NewResult(1, 1))
	mock.ExpectExec(regexp.QuoteMeta("insert into task_results(")).
		WithArgs("task-stderr", 0, "", "runtime warning", false, sqlmock.AnyArg(), sqlmock.AnyArg()).
		WillReturnResult(sqlmock.NewResult(1, 1))
	mock.ExpectCommit()

	mock.ExpectExec(regexp.QuoteMeta("insert into task_events(event_id, task_id, agent_id, event_type, status, body)")).
		WithArgs("evt-stderr", "task-stderr", "agent-1", "report", "failed", sqlmock.AnyArg()).
		WillReturnResult(sqlmock.NewResult(1, 1))

	if err := svc.ProcessTaskReport(req); err != nil {
		t.Fatalf("ProcessTaskReport failed: %v", err)
	}
	updated := svc.GetTask("task-stderr")
	if updated == nil {
		t.Fatalf("task not found in memory")
	}
	if updated.Status != "failed" {
		t.Fatalf("expected status=failed when stderr is non-empty, got %s", updated.Status)
	}
	if _, ok := svc.GetAgent("agent-1").RunningTasks["task-stderr"]; ok {
		t.Fatalf("running task should be removed")
	}
...
func TestRegister_NewAgent_GeneratesToken(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta("insert into agents")).
		WillReturnRows(sqlmock.NewRows([]string{"agent_id"}).AddRow("agent-new"))
...
func TestRegister_DuplicateAgent_UpdatesToken(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta("insert into agents")).
		WillReturnRows(sqlmock.NewRows([]string{"agent_id"}).AddRow("agent-dup"))
...
func TestRegister_TokenUniqueness(t *testing.T) {
	...
	for i := 0; i < 20; i++ {
		mock.ExpectQuery(regexp.QuoteMeta("insert into agents")).
			WillReturnRows(sqlmock.NewRows([]string{"agent_id"}).AddRow("agent-uniq"))
...
// TestRegister_DeviceCodeAgentIDConflict 测试严格绑定：
// device_code 若已绑定到其他 agent_id，注册应返回冲突错误。
func TestRegister_DeviceCodeAgentIDConflict(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatalf("new sqlmock: %v", err)
	}
	...
	svc := service.New(db)
	req := api.RegisterRequest{AgentID: "agent-new", DeviceCode: "dev-fixed"}

	mock.ExpectQuery(regexp.QuoteMeta("insert into agents")).
		WillReturnRows(sqlmock.NewRows([]string{"agent_id"}).AddRow("agent-old"))

	_, err = svc.Register(req, 1*time.Hour, 30*time.Second)
	if !errors.Is(err, service.ErrDeviceCodeAgentIDConflict) {
		t.Fatalf("expected ErrDeviceCodeAgentIDConflict, got %v", err)
	}

	if svc.GetAgent("agent-new") != nil {
		t.Fatalf("agent should not be created in memory on conflict")
	}
