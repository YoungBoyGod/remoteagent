...
func TestUpsertAgent_Success(t *testing.T) {
	...
	if _, err := store.UpsertAgent(db, req, 30, 60); err != nil {
...
func TestUpsertAgent_DefaultTenant(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta("insert into agents")).
		...
		WillReturnRows(sqlmock.NewRows([]string{"agent_id"}).AddRow("agent-2"))

	if _, err := store.UpsertAgent(db, req, 15, 30); err != nil {
		t.Fatalf("UpsertAgent failed: %v", err)
	}
	if err := mock.ExpectationsWereMet(); err != nil {
		t.Fatalf("expectations not met: %v", err)
...
func TestUpsertAgent_ExecError(t *testing.T) {
	...
	_, err = store.UpsertAgent(db, req, 30, 60)
	// 验证错误被正确传播
	if err == nil {
...
func TestUpsertAgent_NilLabelsAndCapabilities(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta("insert into agents")).
		...
		WillReturnRows(sqlmock.NewRows([]string{"agent_id"}).AddRow("agent-nil"))

	if _, err := store.UpsertAgent(db, req, 10, 20); err != nil {
		t.Fatalf("UpsertAgent failed: %v", err)
	}
	if err := mock.ExpectationsWereMet(); err != nil {
		t.Fatalf("expectations not met: %v", err)
