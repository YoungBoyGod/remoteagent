...
func ListTasksV2(ctx context.Context, db *sql.DB, query api.TaskListRequest) ([]TaskRow, int, error) {
	...
	fromClause := "FROM tasks t LEFT JOIN agents a ON a.agent_id = COALESCE(t.agent_id, t.target_agent_id)"
	...
	if query.DeviceCode != "" {
		where += " AND a.device_code ILIKE $" + itoa(idx)
		args = append(args, "%"+query.DeviceCode+"%")
		idx++
	}
	if query.IP != "" {
		where += " AND COALESCE(a.ip::text, '') ILIKE $" + itoa(idx)
		args = append(args, "%"+query.IP+"%")
		idx++
	}
	if query.Submitter != "" {
		where += " AND COALESCE(t.payload->'env'->>'RA_SUBMITTER', t.payload->'env'->>'SUBMITTER', t.payload->'env'->>'CREATED_BY', '') ILIKE $" + itoa(idx)
		args = append(args, "%"+query.Submitter+"%")
		idx++
	}
	if query.CreatedFrom > 0 {
		where += " AND t.created_at >= to_timestamp($" + itoa(idx) + ")"
...
// CountTasksByStatusV2 统计各任务状态总量
func CountTasksByStatusV2(ctx context.Context, db *sql.DB) (map[string]int, int, error) {
	ctx, cancel := context.WithTimeout(ctx, 5*time.Second)
	defer cancel()

	rows, err := db.QueryContext(ctx, `SELECT status, COUNT(*)::int FROM tasks GROUP BY status`)
	if err != nil {
	...
	counts := map[string]int{
		"pending": 0, "leased": 0, "running": 0,
		"success": 0, "failed": 0, "timeout": 0,
		"canceling": 0, "canceled": 0,
	}
	total := 0
	for rows.Next() {
		var status string
		...
		if err := rows.Scan(&status, &cnt); err != nil {
			return nil, 0, fmt.Errorf("scan task status count: %w", err)
		}
		counts[status] = cnt
		total += cnt
	}
	if err := rows.Err(); err != nil {
		return nil, 0, fmt.Errorf("iterate task status counts: %w", err)
	}
