...
func detectExternalIP() string {
	return detectExternalIPWithContext(context.Background(), 5*time.Second)
}

func detectExternalIPWithContext(ctx context.Context, timeout time.Duration) string {
	if timeout > 0 {
...
func scrapeNodeExporter() string {
	return scrapeNodeExporterWithContext(context.Background(), 3*time.Second)
}

func scrapeNodeExporterWithContext(ctx context.Context, timeout time.Duration) string {
	if timeout > 0 {
		var cancel context.CancelFunc
		ctx, cancel = context.WithTimeout(ctx, timeout)
		defer cancel()
	}
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, "http://127.0.0.1:9100/metrics", nil)
	if err != nil {
		return ""
	}
	resp, err := http.DefaultClient.Do(req)
