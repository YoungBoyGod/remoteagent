...
const agentMetaMap = computed(() => {
  const m = new Map<string, DebugAgentItem>()
  for (const a of agents.value) m.set(a.agent_id, a)
  return m
})

function agentDeviceCode(agentID?: string): string {
  if (!agentID) return '-'
  return agentMetaMap.value.get(agentID)?.device_code || '-'
}

function agentIP(agentID?: string): string {
  if (!agentID) return '-'
  return agentMetaMap.value.get(agentID)?.ip || '-'
}
...
async function fetchTasks() {
  ...
  try {
    ...
    if (filter.device_code) params.device_code = filter.device_code
    if (filter.ip) params.ip = filter.ip
    if (filter.submitter) params.submitter = filter.submitter
    if (filter.created_range.length === 2) {
      params.created_from = dayjs(filter.created_range[0]).unix()
      params.created_to = dayjs(filter.created_range[1]).unix()
    }
...
<template>
  <div>
    ...
    <div class="toolbar">
      ...
      <el-input v-model="filter.device_code" placeholder="设备编码" clearable style="width: 150px" @keyup.enter="onSearch" />
      <el-input v-model="filter.ip" placeholder="IP" clearable style="width: 150px" @keyup.enter="onSearch" />
      <el-input v-model="filter.submitter" placeholder="提交人" clearable style="width: 120px" @keyup.enter="onSearch" />
      <el-date-picker
      ...
      <el-tab-pane name="all">
        <template #label>
          <span class="tab-label">
            ...
            全部
            <el-badge :value="statusCounts.all" :max="100000" :hidden="statusCounts.all === 0" class="tab-badge" />
          </span>
        </template>
      </el-tab-pane>
      <el-tab-pane name="pending">
        <template #label>
          <span class="tab-label">
            ...
            排队中
            <el-badge :value="statusCounts.pending" :max="100000" :hidden="statusCounts.pending === 0" type="warning" class="tab-badge" />
          </span>
        </template>
      </el-tab-pane>
      <el-tab-pane name="running">
        <template #label>
          <span class="tab-label">
            ...
            运行中
            <el-badge :value="statusCounts.running" :max="100000" :hidden="statusCounts.running === 0" type="primary" class="tab-badge" />
          </span>
        </template>
      </el-tab-pane>
      <el-tab-pane name="success">
        <template #label>
          <span class="tab-label">
            ...
            已完成
            <el-badge :value="statusCounts.success" :max="100000" :hidden="statusCounts.success === 0" type="success" class="tab-badge" />
          </span>
        </template>
      </el-tab-pane>
      <el-tab-pane name="failed">
        <template #label>
          <span class="tab-label">
            ...
            失败
            <el-badge :value="statusCounts.failed" :max="100000" :hidden="statusCounts.failed === 0" type="danger" class="tab-badge" />
          </span>
        </template>
      </el-tab-pane>
    </el-tabs>
    ...
    >
      ...
      <el-table-column label="设备编码" min-width="160" show-overflow-tooltip>
        <template #default="{ row }">{{ agentDeviceCode(row.agent_id) }}</template>
      </el-table-column>
      <el-table-column label="IP" min-width="140" show-overflow-tooltip>
        <template #default="{ row }">{{ agentIP(row.agent_id) }}</template>
      </el-table-column>
      <el-table-column label="提交人" min-width="120" show-overflow-tooltip>
