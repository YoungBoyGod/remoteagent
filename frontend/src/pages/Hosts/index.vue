...
function sshLoginCmd(row: ManagedHost): string {
  const port = row.port || 22
  const user = row.username || 'root'
  const portFlag = port !== 22 ? ` -p ${port}` : ''
  if (row.auth_type === 'password' && row.password) {
    return `sshpass -p '${row.password}' ssh${portFlag} ${user}@${row.ip}`
  }
  return `ssh${portFlag} ${user}@${row.ip}`
}
...
<template>
  <div>
    ...
    >
      <el-table-column type="expand">
        <template #default="{ row }">
          <div class="expand-content">
            <div class="kv-grid">
              <span class="kv-label">描述</span><span class="kv-value">{{ row.description || '-' }}</span>
              <span class="kv-label">标签</span>
              <span class="kv-value">
                <el-tag v-for="tag in (row.tags || [])" :key="tag" size="small" type="info" effect="plain" style="margin-right:4px">{{ tag }}</el-tag>
                <span v-if="!row.tags?.length">-</span>
              </span>
              <span class="kv-label">创建时间</span><span class="kv-value">{{ formatTime(row.created_at) }}</span>
            </div>
      ...
      <el-table-column label="名称 / 主机名" width="160" show-overflow-tooltip>
        <template #default="{ row }">
          <div class="host-info">
      ...
      <el-table-column label="密码" width="140" align="center">
        <template #default="{ row }">
          <template v-if="row.auth_type === 'password' && row.password">
            <div style="display:flex;align-items:center;gap:4px;justify-content:center">
              <span v-if="revealedPasswords.has(row.host_id)" class="reveal-text" @click="togglePassword(row.host_id)">{{ row.password }}</span>
              <el-button v-else link size="small" @click="togglePassword(row.host_id)">••••••</el-button>
              <el-button :icon="CopyDocument" link size="small" @click="copyText(row.password)" />
            </div>
          </template>
      ...
      <el-table-column label="密钥" width="80" align="center">
        <template #default="{ row }">
          <template v-if="row.auth_type === 'key' && row.ssh_key">
            <el-popover placement="left" :width="400" trigger="click">
              <template #reference>
                <el-button link size="small">查看</el-button>
              </template>
              <pre class="key-preview">{{ row.ssh_key }}</pre>
              <div style="text-align:right;margin-top:8px">
                <el-button size="small" :icon="CopyDocument" @click="copyText(row.ssh_key)">复制</el-button>
              </div>
            </el-popover>
          </template>
          <span v-else class="text-muted">—</span>
      ...
      <el-table-column label="关联 Agent" width="260" show-overflow-tooltip>
        <template #default="{ row }">
          <template v-if="row.agent_id">
            <span class="agent-link" :class="row.agent_status">
              <el-icon size="12"><ConnectionIcon /></el-icon>
              {{ row.agent_id }}
            </span>
          </template>
      ...
      <el-table-column label="连接信息" width="380">
        <template #default="{ row }">
          <div class="conn-item">
            <span class="conn-tag">内网</span>
            <code class="conn-value">{{ sshLoginCmd(row) }}</code>
            <el-button :icon="CopyDocument" link size="small" @click="copyText(sshLoginCmd(row))" />
          </div>
          <div v-if="row.vnc_addr" class="conn-item" style="margin-top:4px">
            <span class="conn-tag">VNC</span>
            <code class="conn-value">{{ row.vnc_addr }}</code>
            <el-button :icon="CopyDocument" link size="small" @click="copyText(row.vnc_addr)" />
          </div>
          <div v-if="row.ext_ssh_addr" class="conn-item" style="margin-top:4px">
            <span class="conn-tag ext">外网</span>
            <code class="conn-value">{{ row.ext_ssh_addr }}</code>
            <el-button :icon="CopyDocument" link size="small" @click="copyText(row.ext_ssh_addr)" />
          </div>
          <div v-if="row.ext_vnc_addr" class="conn-item" style="margin-top:4px">
            <span class="conn-tag ext">外VNC</span>
            <code class="conn-value">{{ row.ext_vnc_addr }}</code>
            <el-button :icon="CopyDocument" link size="small" @click="copyText(row.ext_vnc_addr)" />
          </div>
        </template>
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="760px" destroy-on-close>
      <el-form ref="formRef" :model="form" :rules="formRules" label-width="100px">
        ...
        <el-form-item v-if="form.auth_type === 'password'" label="密码">
          <el-input v-model="form.password" :placeholder="editingHostId ? '留空则不修改' : '请输入密码'" />
        </el-form-item>
