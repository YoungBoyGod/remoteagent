...
func (a *Agent) sendHeartbeat(ctx context.Context) error {
	...
	// Keep heartbeat responsive: best-effort metrics collection with short deadlines.
	nodeMetrics := scrapeNodeExporterWithContext(ctx, 100*time.Millisecond)
	if nodeMetrics != "" {
	...
	req := heartbeatRequest{
		...
		ExternalIP:        detectExternalIPWithContext(ctx, 100*time.Millisecond),
	}
