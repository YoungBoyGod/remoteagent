...
// TestRegisterOnce_AgentIDMismatch 验证注册响应返回不同 agent_id 时应拒绝，保持本地 agent_id 不变
func TestRegisterOnce_AgentIDMismatch(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		data, _ := json.Marshal(registerData{
	...
	err := a.registerOnce(context.Background())
	if err == nil {
		t.Fatal("expected agent_id mismatch error")
	}
	if !strings.Contains(err.Error(), "agent_id mismatch") {
		t.Fatalf("unexpected error: %v", err)
	}
	if a.agentID != oldID {
		t.Fatalf("agent_id changed unexpectedly: got %s want %s", a.agentID, oldID)
	}
}
