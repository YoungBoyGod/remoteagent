...
func (a *Agent) enqueueLocal(task localTask) error {
	...
	_, err := a.db.ExecContext(ctx, `
		INSERT INTO local_tasks(task_id, server_task_id, task_type, status, exec_mode, priority, payload, leased_until_ms, queued_at_ms, attempt)
		VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	...
	`, task.TaskID, task.ServerTaskID, task.TaskType, task.Status, task.ExecMode,
		task.Priority, task.Payload, task.LeasedUntilMs, task.QueuedAtMs, task.Attempt)
	return err
}
